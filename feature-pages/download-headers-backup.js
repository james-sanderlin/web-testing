...
function initializeDownloadHeadersTest() {
  ...
  function showServerSetupInstructions() {
    const instructions = `
To test real HTTP headers, you need a proper Node.js server instead of the static file server.

Quick setup:
1. npm install express
2. Create server.js with Express routes
3. npm start to run the Express server

The API endpoint at /api/download-test would then work properly with real headers.
    `.trim();
    
    addResult('info', instructions, new Date().toLocaleTimeString());
  }
  ...
  function performRealServerDownload(config, downloadOption, contentDisposition, testId) {
    ...
    // Create download link pointing to server endpoint
    ...
    // Show what headers are being set
    const headerInfo = {
      'Content-Type': config.mimeType,
  ...
  function performAssetFileDownload(config, downloadOption, contentDisposition, testId) {
    try {
      // For DOCX, download the actual asset file
      ...
      // Create download link to actual asset
      const link = document.createElement('a');
      link.href = assetUrl;
      link.download = filename;
      
      // Add headers simulation note
      ...
      // Show what headers would be set in a real server
      const headerInfo = {
        'Content-Type': config.mimeType,
        'Content-Disposition': getContentDispositionHeader(contentDisposition, filename),
        'X-Download-Options': downloadOption === 'none' ? 'Not Set' : downloadOption,
  ...
  function performFallbackDownload(config, downloadOption, contentDisposition, testId) {
    // For Word documents, try to use the actual asset file
    if (config.extension === '.docx') {
      return performAssetFileDownload(config, downloadOption, contentDisposition, testId);
    }
    
    // Create a URL for the server endpoint (for documentation purposes)
    const params = new URLSearchParams({
      file: config.extension.substring(1), // Remove the dot
      headers: downloadOption,
      disposition: contentDisposition,
    ...
    addResult('info', `Server endpoint would be: ${serverUrl}`, new Date().toLocaleTimeString());
    addResult('info', `Since we're using static file server, falling back to asset file download`, new Date().toLocaleTimeString());
    
    // Fallback to direct download with better simulation
...
- Timestamp: ${Date.now()}
- Test Configuration: Check console for details
...
/F1 12 Tf
100 700 Td
(X-Download-Options Test PDF) Tj
0 -20 Td
